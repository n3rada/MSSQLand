...
namespace MSSQLand.Services
{
    internal class AdsiService
    {
        ...
        {
            serverName = $"ADSI_{ByteHelper.GetRandomIdentifier(6)}";
        ...
        private string[] GetLdapServerAssembly()
        {
            ...
            // Which is finally: https://raw.githubusercontent.com/blackarrowsec/redteam-research/2b1cdcb67a490880ce0b0714983cbd707cb8b770/MSSQL%20linked%20servers%20-%20ADSI/ldapServer.cs
            // This add 17kB of compiled data
            ...
            // Decode and decompress back to original bytes, then compute hash + hex via shared utility.
            byte[] decompressedBytes = ByteHelper.GzipDecompress(Convert.FromBase64String(compressedBase64));
            return ByteHelper.ConvertDllToSqlBytes(decompressedBytes);
        }
    }
}
