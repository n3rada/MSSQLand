...
namespace MSSQLand.Services
{
    internal class AdsiService
    {
        ...
        public string FunctionName = "ldap_listener";
        ...
        public Task<DataTable> ListenForRequest()
        {
            ...
            Task.Run(() =>
            {
                try
                {
                    ...
                    Logger.Task($"Starting a local LDAP server on port {Port}");
        ...
        public bool LoadLdapServerAssembly()
        {
            ...
            catch (Exception ex)
            {
                ...
                Logger.InfoNested($"Deleting LDAP server assembly '{dropAssembly}', function '{dropFunction}' and trusted assembly hash '{dropClrHash}'");
                _databaseContext.QueryService.ExecuteNonProcessing(dropFunction);
                _databaseContext.QueryService.ExecuteNonProcessing(dropAssembly);
                _databaseContext.QueryService.ExecuteNonProcessing(dropClrHash);
                return false;
        ...
        private string[] GetLdapServerAssembly()
        {
            ...
            byte[] decompressedBytes = DataProcessor.DecodeAndDecompress(compressedBase64);
            string decompressedHex = DataProcessor.BytesToHexString(decompressedBytes);
