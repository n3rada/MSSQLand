...
WHERE sys.Name0 = '{_deviceName.Replace("'", "''")}';";
                ...
                // Get boundary groups and distribution points for this device
                Logger.NewLine();
                Logger.Info("Boundary Groups & Distribution Points");
                Logger.InfoNested("Where can this device get content from?");

                string deviceIP = device["IPAddress"]?.ToString()?.Split(',')[0] ?? "";
                string deviceADSite = device["ADSite"]?.ToString() ?? "";

                // Use BGB_LiveData_Boundary which tracks actual device-to-boundary-group assignments
...
FROM [{db}].dbo.BGB_LiveData_Boundary blb
INNER JOIN [{db}].dbo.vSMS_BoundaryGroup bg ON blb.BoundaryGroupID = bg.GroupID
...
ORDER BY bg.Name;";
                ...
                try
                {
                    DataTable boundaryResult = databaseContext.QueryService.ExecuteTable(boundaryQuery);

                    if (boundaryResult.Rows.Count > 0)
                    {
                        Logger.InfoNested($"Device IP: {deviceIP}, AD Site: {deviceADSite}");
                        Console.WriteLine(OutputFormatter.ConvertDataTable(boundaryResult));

                        var groupIds = string.Join(",", boundaryResult.AsEnumerable()
...
SELECT DISTINCT
    bg.Name AS BoundaryGroupName,
    ss.ServerName AS DistributionPoint,
    CASE ss.RoleName
        ...
        ELSE ss.RoleName
    END AS Role
FROM [{db}].dbo.vSMS_BoundaryGroupSiteSystems bgss
INNER JOIN [{db}].dbo.vSMS_BoundaryGroup bg ON bgss.GroupID = bg.GroupID
...
ORDER BY bg.Name, ss.ServerName;";
                            ...
                            Logger.NewLine();
                            if (dpResult.Rows.Count > 0)
                            {
                                Logger.Info("Available Distribution Points");
                                Console.WriteLine(OutputFormatter.ConvertDataTable(dpResult));
                                Logger.Success($"Device can access {dpResult.Rows.Count} distribution point(s)");
                            }
                            else
                            {
                                Logger.Warning("No distribution points configured for assigned boundary groups");
                            }
                        }
                    }
                    else
                    {
                        Logger.Warning($"Device not tracked in any boundary group (IP: {deviceIP}, AD Site: {deviceADSite})");
                        Logger.WarningNested("Device may not have reported location yet or boundaries not configured");
                    }
                }
                catch (Exception ex)
                {
                    Logger.Warning($"Could not query boundaries: {ex.Message}");
                    Logger.InfoNested($"Device IP: {deviceIP}, AD Site: {deviceADSite}");
                }
